"""直接用原始 SQL 在 MySQL 中重建全部 18 张表（对应 docs/database-schema.md）。

不用 ORM 映射：SET FOREIGN_KEY_CHECKS=0 后 DROP 再 CREATE，按依赖顺序建表。
"""
import pymysql
...
CREATE_STATEMENTS = [
    ...
    """,
    # 2.9 favorites（多态，target_id 不建物理外键）
    """
    ...
    """,
    # 2.13 question_knowledge_points
    """
...
TABLES = [
    "qa_history", "wrong_questions", "exam_records", "exam_questions", "exams",
    "question_knowledge_points", "questions", "knowledge_points", "learning_calendar",
    "favorites", "notes", "learning_records", "enrollments", "courseware",
    "chapters", "courses", "categories", "users",
]
