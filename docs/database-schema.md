...
> **本文档为导出物，schema 唯一真源是 `backend/app/models/` 的 SQLAlchemy 模型**（自 2026-07-17 起，初始迁移 `6a1c0f34e2a6` 由 ORM autogenerate 生成，废弃手写 DDL）。两者不一致时以模型为准，并回头修订本文档。
> 索引/约束名由 `app/core/database.py` 的 naming_convention 确定性生成（`ix_*`/`uq_*`/`fk_*`/`pk_*`）；主键与外键统一签名 `BIGINT`（不再使用 UNSIGNED）。

- 数据库：**MySQL 8.0**（业务数据，`utf8mb4` / `utf8mb4_unicode_ci`）、**Redis 7.0**（会话缓存/限流/临时数据）、**ChromaDB**（课程知识向量，持久化）。
- 命名规范：**表名小写复数**；**字段下划线分隔**；**主键 `id` 自增（BIGINT）**；**外键 `{表单数}_id`**。
...
| parent_id | BIGINT | FK→categories.id, NULL | 支持二级分类，自关联 |
...
| category_id | BIGINT | FK→categories.id, NULL | **分类（补全）** |
| teacher_id | BIGINT | FK→users.id, NOT NULL | 授课教师 |
| status | ENUM('draft','published','offline') | NOT NULL, DEFAULT 'draft' | 上架/下架 |
| price | DECIMAL(10,2) | DEFAULT 0.00 | 预留 |
| student_count | INT | DEFAULT 0 | 选课人数冗余（可缓存） |
| created_at / updated_at | DATETIME | | |
...
| parent_id | BIGINT | FK→chapters.id, NULL | 树形结构 |
| title | VARCHAR(200) | NOT NULL | |
...
|------|------|------|------|
| id | BIGINT | PK, AI | |
| chapter_id | BIGINT | FK→chapters.id, NOT NULL | 所属章节 |
| title | VARCHAR(200) | NOT NULL | |
...
| file_size | BIGINT | NULL | 字节数（补全） |
| mime_type | VARCHAR(128) | NULL | MIME（补全） |
| duration | INT | NULL | 视频时长(秒)，video 专用（补全） |
| sort_order | INT | DEFAULT 0 | |
| uploaded_by | BIGINT | FK→users.id, NULL | 上传者（补全） |
| created_at / updated_at | DATETIME | | |

**索引**：INDEX(`chapter_id`)、INDEX(`type`)、INDEX(`uploaded_by`)。
...
| course_id | BIGINT | FK→courses.id, NOT NULL | |
| courseware_id | BIGINT | FK→courseware.id, NOT NULL | 具体课件 |
| last_position | INT | DEFAULT 0 | 视频断点(秒) |
| progress_percent | DECIMAL(5,2) | DEFAULT 0.00 | 该课件进度 |
| duration_watched | INT | DEFAULT 0 | 累计观看时长(秒) |
| is_completed | TINYINT(1) | DEFAULT 0 | |
| last_learned_at | DATETIME | NULL | 最近学习时间 |
| created_at / updated_at | DATETIME | | |

**索引**：**UNIQUE(`user_id`,`courseware_id`)**、INDEX(`user_id`,`course_id`)、INDEX(`course_id`)、INDEX(`courseware_id`)。
...
| user_id | BIGINT | FK→users.id, NOT NULL | |
| course_id | BIGINT | FK→courses.id, NOT NULL | |
| chapter_id | BIGINT | FK→chapters.id, NULL | |
| courseware_id | BIGINT | FK→courseware.id, NULL | 关联课件 |
| content | TEXT | NOT NULL | 笔记内容 |
| video_timestamp | INT | NULL | 关联视频时间点(秒) |
| created_at / updated_at | DATETIME | | |

**索引**：INDEX(`user_id`)、INDEX(`course_id`)、INDEX(`courseware_id`)。
...
| target_id | BIGINT | NOT NULL | 对象 id（逻辑外键，随 type 变） |
| created_at | DATETIME | | |
...
| duration_seconds | INT | DEFAULT 0 | 当日学习总时长 |
| courseware_count | INT | DEFAULT 0 | 当日学习课件数 |
| note_count | INT | DEFAULT 0 | 当日笔记数 |
| created_at / updated_at | DATETIME | | |
...
| parent_id | BIGINT | FK→knowledge_points.id, NULL | 可树形 |
...
| created_by | BIGINT | FK→users.id, NULL | 出题教师 |
| created_at / updated_at | DATETIME | | |

**索引**：INDEX(`course_id`)、INDEX(`type`)、INDEX(`created_by`)。
...
| knowledge_point_id | BIGINT | FK→knowledge_points.id, NOT NULL | |
...
| title | VARCHAR(200) | NOT NULL | |
| description | TEXT | NULL | |
| duration_minutes | INT | NOT NULL | 限时(分钟) |
...
**索引**：INDEX(`course_id`)、INDEX(`status`)、INDEX(`created_by`)。
...
| id | BIGINT | PK, AI | |
| exam_id | BIGINT | FK→exams.id, NOT NULL | |
| user_id | BIGINT | FK→users.id, NOT NULL | 考生 |
...
| user_id | BIGINT | FK→users.id, NOT NULL | |
| question_id | BIGINT | FK→questions.id, NOT NULL | |
| exam_record_id | BIGINT | FK→exam_records.id, NULL | 来源考试记录 |
...
**索引**：**UNIQUE(`user_id`,`question_id`)**、INDEX(`question_id`)。
...
| id | BIGINT | PK, AI | |
| user_id | BIGINT | FK→users.id, NOT NULL | |
| course_id | BIGINT | FK→courses.id, NULL | 问答上下文课程 |
